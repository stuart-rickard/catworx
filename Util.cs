...

{
  class Util
  {
    ...
    {
      Console.WriteLine("Making CSV file and Badges for these Employees: ");

      for (int i = 0; i < employees.Count; i++)
      {
        string template = "{0,-10}\t{1,-20}";
        Console.WriteLine(String.Format(template, employees[i].GetId(), employees[i].GetFullName()));
      }
    }
    ...
    async public static Task MakeBadges(List<Employee> employees)
    {
      ...
      using (HttpClient client = new HttpClient())
      {
        for (int i = 0; i < employees.Count; i++)
        {
          ...
          data.SaveTo(File.OpenWrite("data/employee_" + employees[i].GetId().ToString() + "_Badge.png"));
        }
      }
    }
  }
